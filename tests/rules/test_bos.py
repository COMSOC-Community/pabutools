...
from pabutools.election import (Project, Instance, ApprovalBallot, ApprovalProfile, Cost_Sat, CardinalBallot,
                                CardinalProfile)
from pabutools.rules.mes.bos_equal_shares import bos_equal_shares, fractional_equal_shares
...
class TestEqualShares(unittest.TestCase):
    ...
    def test_budget_constraint(self):
        ...
        self.assertLessEqual(total_cost, instance.budget_limit)

    def test_tail_utilities(self):
        pA = Project("A", 1)
        pB = Project("B", 1)
    ...
    def test_budget_constraint_with_EJR(self):
        p1 = Project("p1", 600)
        p2 = Project("p2", 600)
        instance = Instance([p1, p2], 1000)
    ...
    def test_bos_basic_logic_with_EJR(self):
        p1, p2 = Project("p1", 1000), Project("p2", 100)
        instance = Instance([p1, p2], 1000)
        profile = ApprovalProfile([ApprovalBallot({p1}), ApprovalBallot({p2}), ApprovalBallot({p1})])
    ...
    def test_empty_instance_with_EJR(self):
        instance = Instance([], 1000)
        profile = ApprovalProfile([])
    ...
    def test_over_budget_projects_with_EJR(self):
        p1 = Project("Overpriced", 2000)
        instance = Instance([p1], 1000)
        profile = ApprovalProfile([ApprovalBallot({p1})])
